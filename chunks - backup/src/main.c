...
void drawAllBlocks (node *blocks, CHUNK_MANAGER *chunks, GLuint shader, GLuint VAOID[]){
	...
	player.camera.model = (mat4){(vec4){1.0, 0.0, 0.0, 0.0}, (vec4){0.0, 1.0, 0.0, 0.0}, (vec4){0.0, 0.0, 1.0, 0.0}, (vec4){0.0, 0.0, 0.0, 1.0}};
	player.camera.model = translate_mat4_vec3 (player.camera.model, blocks->data.pos);
...
BLOCK firstBlock (CHUNK_MANAGER *chunks, ray r) {
	...
	for (int i = 0; i < 1331; i++, aux = base) {
		...
		b = get_element_clone (*relativePos (chunks, aux), vecToBlock(aux));
...
int isBlock (TREE *tree, vec3 pos) {
	return get_block (T, vecToBlock(pos));
}
	...
	for (int i = 0; i < 3; i++)
		for (int j = 0; j < 3; j++)
			for (int k = 0; k < 3; k++) {
				...
				destroyChunk(state->chunks_info->chunks[i][j][k].blocks);
				state->chunks_info->chunks[i][j][k].blocks = NULL;
			}
...
void saveChunk (CHUNK *chunk) {
	...
	saveBlocks (chunk->blocks, p);
	fclose(p);
	
	free (filePath);
}

void saveBlocks (node *chunk, FILE *p) {
	
    if (!chunk)
        return;
	
    fprintf(p, "%f %f %f %d %hd %hd %hd %hd %hd %d %d %d %d \n", chunk->data.pos.x, chunk->data.pos.y, chunk->data.pos.z, chunk->data.id, chunk->data.north, chunk->data.south, chunk->data.east, chunk->data.west, chunk->data.power, chunk->data.powered, chunk->data.power_source, chunk->data.lit, chunk->data.tick);
    
    saveBlocks (chunk->left, p);
    saveBlocks (chunk->right, p);
}
...
void insertBlockToChunk (CHUNK_MANAGER *chunks, BLOCK b) {
	insertBlock (relativePos(chunks, b.pos), b);
}

void removeBlockToChunk (CHUNK_MANAGER *chunks, BLOCK b) {
	removeBlock (relativePos(chunks, b.pos), b);
}

BLOCK *get_element (node *T, BLOCK data) {
	while (T) {
...
BLOCK get_element_clone (node *T, BLOCK data) {
	while (T) {
		if (compareBlocks(data, T->data))
			T = T->left;
		else if (compareBlocks(T->data, data))
